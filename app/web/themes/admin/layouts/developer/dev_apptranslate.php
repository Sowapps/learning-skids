...
<div class="row">
	<div class="col-lg-8">
		...
		<p><?php _t('notifiyDeveloperFromUpdates', DOMAIN_TRANSLATIONS, array('DEVEMAIL'=>DEVEMAIL, 'APP_NAME'=>t('app_name'))); ?></p>
			...
			<div class="panel <?php echo isset($editedDomains[$domain]) ? 'panel-info' : 'panel-default'; ?>">
				<div class="panel-heading" role="tab" id="#Domain<?php echo $domain; ?>Heading">
					<h4 class="panel-title">
						<a role="button" data-toggle="collapse" data-parent="#TranslatedDomains" href="#Domain<?php echo $domain; ?>" aria-expanded="false" aria-controls="Domain<?php echo $domain; ?>">
							<?php
							echo $domain.($missing ? ' <small>'.t('missingXTranslations', DOMAIN_TRANSLATIONS, $missing).'</small>' : '');
// 							echo (isset($publicDomains[$domain]) ? '<i class="fa fa-star mr4"></i>' : '').$domain.($missing ? ' <small>'.t('missingXTranslations', DOMAIN_TRANSLATIONS, $missing).'</small>' : '');
							?>
						</a>
					</h4>
				</div>
...
$(function() {
	provideTranslations(<?php echo json_encode(array(
